...
#import "TVUMJRefreshAutoFooter.h"

@interface TVUMJRefreshAutoFooter()
/** 一个新的拖拽 */
@property (assign, nonatomic, getter=isOneNewPan) BOOL oneNewPan;
@end

@implementation TVUMJRefreshAutoFooter
...
- (void)scrollViewContentOffsetDidChange:(NSDictionary *)change
{
    ...
    if (self.state != TVUMJRefreshStateIdle || !self.automaticallyRefresh || self.mj_y == 0) return;
...
- (void)scrollViewPanStateDidChange:(NSDictionary *)change
{
    ...
    if (self.state != TVUMJRefreshStateIdle) return;
...
- (void)setState:(TVUMJRefreshState)state
{
    ...
    if (state == TVUMJRefreshStateRefreshing) {
        [self executeRefreshingCallback];
    } else if (state == TVUMJRefreshStateNoMoreData || state == TVUMJRefreshStateIdle) {
